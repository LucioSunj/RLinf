...
def get_env_cls(env_type: str, env_cfg=None):
    ...
    if env_type == SupportedEnvType.MANISKILL:
        if env_cfg.get("enable_offload", False):
            from Reinforcement_Learning.RLinf.rlinf.envs.maniskill.maniskill_offload_env import ManiskillOffloadEnv

            return ManiskillOffloadEnv
        else:
            from Reinforcement_Learning.RLinf.rlinf.envs.maniskill.maniskill_env import ManiskillEnv

            return ManiskillEnv
    elif env_type == SupportedEnvType.LIBERO:
        from Reinforcement_Learning.RLinf.rlinf.envs.libero.libero_env import LiberoEnv

        return LiberoEnv
    elif env_type == SupportedEnvType.ROBOTWIN:
        from Reinforcement_Learning.RLinf.rlinf.envs.robotwin.robotwin_env import RoboTwinEnv

        return RoboTwinEnv
    elif env_type == SupportedEnvType.ISAACLAB:
        from Reinforcement_Learning.RLinf.rlinf.envs.isaaclab import REGISTER_ISAACLAB_ENVS
        ...
        from Reinforcement_Learning.RLinf.rlinf.envs.metaworld.metaworld_env import MetaWorldEnv

        return MetaWorldEnv
    elif env_type == SupportedEnvType.BEHAVIOR:
        from Reinforcement_Learning.RLinf.rlinf.envs.behavior.behavior_env import BehaviorEnv

        return BehaviorEnv
    elif env_type == SupportedEnvType.CALVIN:
        from Reinforcement_Learning.RLinf.rlinf.envs.calvin.calvin_gym_env import CalvinEnv

        return CalvinEnv
    elif env_type == SupportedEnvType.ROBOCASA:
        from Reinforcement_Learning.RLinf.rlinf.envs.robocasa.robocasa_env import RobocasaEnv

        return RobocasaEnv
    elif env_type == SupportedEnvType.REALWORLD:
        from Reinforcement_Learning.RLinf.rlinf.envs.realworld.realworld_env import RealWorldEnv

        return RealWorldEnv
    elif env_type == SupportedEnvType.HABITAT:
        from Reinforcement_Learning.RLinf.rlinf.envs.habitat.habitat_env import HabitatEnv

        return HabitatEnv
    elif env_type == SupportedEnvType.FRANKASIM:
        from Reinforcement_Learning.RLinf.rlinf.envs.frankasim.frankasim_env import FrankaSimEnv

        return FrankaSimEnv
    elif env_type == SupportedEnvType.OPENSORAWM:
        from Reinforcement_Learning.RLinf.rlinf.envs.world_model.world_model_opensora_env import OpenSoraEnv

        return OpenSoraEnv
    elif env_type == SupportedEnvType.WANWM:
        from Reinforcement_Learning.RLinf.rlinf.envs.world_model.world_model_wan_env import WanEnv
