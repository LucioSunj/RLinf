...
from Reinforcement_Learning.RLinf.rlinf.utils.logging import get_logger
from Reinforcement_Learning.RLinf.rlinf.utils.metric_logger import MetricLogger
from Reinforcement_Learning.RLinf.rlinf.utils.metric_utils import compute_evaluate_metrics, print_metrics_table
from Reinforcement_Learning.RLinf.rlinf.utils.runner_utils import check_progress

if TYPE_CHECKING:
    from Reinforcement_Learning.RLinf.rlinf.workers.actor.async_fsdp_sac_policy_worker import (
        AsyncEmbodiedSACFSDPPolicy,
    )
    from Reinforcement_Learning.RLinf.rlinf.workers.actor.fsdp_actor_worker import EmbodiedFSDPActor
    from Reinforcement_Learning.RLinf.rlinf.workers.actor.fsdp_sac_policy_worker import EmbodiedSACFSDPPolicy
    from Reinforcement_Learning.RLinf.rlinf.workers.env.async_env_worker import AsyncEnvWorker
    from Reinforcement_Learning.RLinf.rlinf.workers.env.env_worker import EnvWorker
    from Reinforcement_Learning.RLinf.rlinf.workers.rollout.hf.async_huggingface_worker import (
        AsyncMultiStepRolloutWorker,
    )
    from Reinforcement_Learning.RLinf.rlinf.workers.rollout.hf.huggingface_worker import MultiStepRolloutWorker
...
class EmbodiedRunner:
    ...
    def run(self):
        ...
        for _step in range(start_step, self.max_steps):
            # set global step 当前训练步广播给 actor 和 rollout。某些组件会用它做日志、checkpoint 命名、温度或调度控制
            ...
            with self.timer("step"):
                with self.timer("sync_weights"):
                    if _step % self.weight_sync_interval == 0:
                        self.update_rollout_weights() # on-policy rl: 保证 rollout 和 actor policy 权重相同
                with self.timer("generate_rollouts"):
                    ...
                    env_handle: Handle = self.env.interact(
                        input_channel=self.rollout_channel,
                        output_channel=self.env_channel,
                    ) # env worker 开始和环境交互，通过 channel 收动作、发 observation/reward/done。
