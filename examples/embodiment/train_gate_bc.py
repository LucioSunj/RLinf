...
Example:
  ...
  python examples/embodiment/train_gate_bc.py \
    --labels '/path/to/gate_oracle/libero_idm/shard_*.pt' \
    --out /path/to/ckpts/gate_bc_libero_idm.pt \
    --epochs 30 --device cuda

Then in the GRPO config (see adaptive_gate_README.md §M3):
  runner.ckpt_path: /path/to/ckpts/gate_bc_libero_idm.pt
  actor.model.gate.bc_init_path: /path/to/ckpts/gate_bc_libero_idm.pt
  actor.model.gate.kl_prior.enabled: True
  actor.model.gate.kl_prior.beta: 0.05
  actor.model.gate.kl_prior.decay_steps: 200
...
def main():
    ...
    ap.add_argument("--num-modes", type=int, default=2,
                    help="must be 2: UNCOND=0, IDM=1")
    ...
    ap.add_argument("--class-weight-power", type=float, default=0.0,
                    help="opt-in inverse-frequency CE weighting; changes calibration")
    ...
    ap.add_argument("--max-best-err", type=float, default=None,
                    help="drop rows where even the best mode exceeds this error")
    ap.add_argument("--max-idm-err", type=float, default=None,
                    help="drop rows where the full IDM reference exceeds this error")
    ap.add_argument("--quality-warn-threshold", type=float, default=0.5,
                    help="report/warn fraction whose best absolute error exceeds this")
    args = ap.parse_args()
    if args.num_modes != 2:
        ap.error("--num-modes must be 2 (UNCOND=0, IDM=1)")
    ...
    dataset = GateOracleLabelDataset.from_shards(
        args.labels,
        relabel=relabel,
        max_best_err=args.max_best_err,
        max_idm_err=args.max_idm_err,
    ...
    print(f"loaded {len(dataset)} labeled states "
          ...
          f"text_feat_dim={dataset.text_feat_dim})")
    ...
    meta = {
        "kind": "gate_bc",
        "labels": args.labels,
        "relabel": dataset.meta.get("relabel"),
    ...
    print("wire into GRPO: runner.ckpt_path / actor.model.gate.bc_init_path "
          "(+ gate.kl_prior.enabled/beta/decay_steps for actor-side KL regularization)")
