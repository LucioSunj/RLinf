#!/usr/bin/env bash
# SETTING: Five GRPO fine-tunes initialized from G-uplift on held-out LIBERO-Plus Gate-Train, with a decaying actor-side KL prior.
# MODEL/CHECKPOINT LINEAGE: final frozen S-DR + final all-data G-uplift -> independent G-uplift-to-GRPO checkpoints per seed.
# SCIENTIFIC GOAL: Correct proxy bias with closed-loop task reward while retaining the counterfactual allocation warm start.
...
# REQUIRED INPUTS: UPLIFT_CKPT, shared WAM artifacts, disjoint Gate manifests, and PASS E4_DECISION.
...
KL_DECAY_STEPS=${KL_DECAY_STEPS:-200}
...
RUN_DIR="${EXPERIMENT_ROOT}/E5_uplift_grpo/${RUN_ID}"
...
for seed in "${SEED_ARRAY[@]}"; do
    ...
    EXPERIMENT_NAME="e5_uplift_grpo_seed_${seed}"
    ...
    OVERRIDES=(
        ...
        "actor.model.gate.kl_prior.path=${UPLIFT_CKPT}"
        "actor.model.gate.kl_prior.beta=${KL_BETA}"
        "actor.model.gate.kl_prior.beta_end=0.0"
        "actor.model.gate.kl_prior.decay_steps=${KL_DECAY_STEPS}"
    ...
    RUN_ARTIFACTS=(
        ...
        "${DATASET_STATS}" "${COST_PROFILE}" "${GATE_TRAIN_MANIFEST}"
        "${GATE_VAL_MANIFEST}" "${PLUS_FULL_MANIFEST}" "${E4_DECISION}"
    )
